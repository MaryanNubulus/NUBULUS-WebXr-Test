/**
 * PeixosExperience - Experiència amb plane geometry
...
export class PeixosExperience extends ExperienceManager {
  constructor(config) {
    super({ type: 'image-tracking', name: 'Peixos' });
    this.config = config;
    this.tracker = null;
  ...
  async init() {
    ...
    try {
      ...
      // Crea plane amb aspect ratio del vídeo
      ...
      this.tracker.onTargetFound(0, () => {
        console.log('Peixos target found');
        this.videoTexture.play();
      });

      this.tracker.onTargetLost(0, () => {
        console.log('Peixos target lost');
        this.videoTexture.pause();
      });

      this.isInitialized = true;
    } catch (error) {
      console.error('Error initializing PeixosExperience:', error);
      throw error;
    }
  }
  ...
  addAnimations() {
    ...
    const animate = () => {
      ...
      time += 0.01;
      this.mesh.rotation.z = Math.sin(time * 0.3) * 0.01;
